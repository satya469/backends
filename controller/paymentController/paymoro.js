...
exports.paymorodepositRequest = (req, res, next, subIssuingBank, bankRequestUrl, mainamount, configs) => {
    ...
    if (req.headers["user-device"] == "telegram") {
        returnUrl = configdata.telegramreturnurl
    }
    ...
    let bodydata = JSON.stringify({
        ...
        "customerEmail": BASECON.validateEmailType(useritem.email) ? useritem.email : "steve@igamez.ai" ,
...
exports.paymoroWallet = async (req, res, next) => {
    ...
    const { paymentmenuid, amount, depositBankCode } = req.body.params;
    let _id = req.user._id;    var paymoroconfig = await PaymentMenuModel.findOne({ _id: paymentmenuid }).populate("paymentconfigurationid");
    if (paymoroconfig) {
...
exports.paymoroNotify = async (req, res, next) => {    await BASECON.paymentlog({
        type: "paymorodeposit",
        data: req.body,
    })
    ...
    var userdata = await BASECON.playerFindbyUseridUpdate(txnhis.userid);
    if (txnhis && userdata) {
        let rate = await BASECON.getExchangeRate();        let amount = parseInt(txnhis.realamount) * rate;        if (transactionStatus == "SUCCESS") {
        ...
        this.transactionUpdate(updata, ipaddress);
    } else {
...
exports.transactionUpdate = async (data, ipaddress) => {
    ...
    if (data.status == PCONFIG.Approve) {
        ...
        if (!firsttrans) {
            ...
            this.friendInviteBonus(data, ipaddress)
            let bdata = await BonusConfig.findOne({ type: "bonus" });
            if (bdata && bdata.status && bdata.bonusid) {
                Playercontrol.bonusoptions(data.amount, data.userid, bdata.bonusid);
            } else {
...
exports.friendInviteBonus = async (transhis, ipaddress) => {
    let isFriend = await friendModel.findOne({ UserId: mongoose.Types.ObjectId(transhis.userid) }).populate("FriendUserId");    if (isFriend) {
        var mainamount = transhis.amount;
        let config = await firstpagesetting.findOne({ type: "Friendly" });
        if (config) {
            let cc = config.content;            var amount = 0;
            ...
            row["lastbalance"] = BASECON.getPlayerBalanceCorrect(isFriend.FriendUserId.playerid.balance);
