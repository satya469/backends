...
exports.report_bygameid_history = async (req, res, next) => {
    ...
    if (orquery.length > 0) {
        ...
        if (providerid && providerid.length > 0) {
            andqueryobj['providerid'] = mongoose.Types.ObjectId(providerid)
        }
        ...
        if (array && array.length > 0) {
            ...
            for (var i in array) {
                ...
                // wallet["BET"] = wallet.BET;
                // wallet["GGR"] = wallet.BET - wallet.WIN;
...
exports.report_bygameid_total = async (req, res, next) => {
    ...
    if (orquery.length > 0) {
        ...
        if (array && array.length > 0) {
            ...
            // totalbet = totalwallet.BET;
            // totalggr = totalwallet.BET - totalwallet.WIN;
            totalcancel = totalwallet.CANCELED_BET;
    ...
    res.json(
        {
            status: true,
            data: {
                totalwin, totalbet, totalggr, realplayerscount,
                gameoptions,
                provideroptions
            },
        })
...
exports.report_bybet_history = async (req, res, next) => {
    ...
    var andquery = { "DATE": { $gte: start, $lte: end }, AMOUNT: { $ne: 0 } }
    var role = BASECONTROL.getUserItem(req);

    if (userid && userid.length > 0) {
        let useritem = await BASECONTROL.BfindOne(UserModel, { _id: userid });
        if (useritem) {
            andquery['userid'] = mongoose.Types.ObjectId(userid)
    ...
    if (totalcount > 0) {
        ...
        array = await bethistory_model.find(andquery, "AMOUNT TYPE DATE ipaddress transactionId").populate("gameid", ["TYPE", "NAME", "ID", "PROVIDERID", "image"])
            .populate("userid").skip(pages.skip).limit(pages.params.perPage);
...
exports.report_bybet_total = async (req, res, next) => {
    ...
    //     let useritem = await BASECONTROL.BfindOne(UserModel, { _id: userid });
    //     if (useritem) {
    //         andquery = [{ "DATE": { $gte: start, $lte: end }, AMOUNT: { $ne: 0 }, userid: mongoose.Types.ObjectId(userid) }]
...
exports.adminBetsHitoryTotalFromEmail = async (req, res, next) => {
    ...
    var newrow = Object.assign({}, { totalbet: parseInt(totalwallet.BET) },
        { totalwin: parseInt(totalwallet.WIN) },
        { currentwalletbalance: parseInt(currentwalletbalance) },
        { totaldeposit: MakingDepositsAmount });
...
exports.adminreports_email_load = async (req, res, next) => {
    ...
    if (totalcount > 0) {
        var array = await bethistory_model.find(andquery)
            .sort({ DATE: -1 })
            .populate({ path: "gameid", select: "PROVIDERID NAME ID" })
...
exports.sportsBygameshistory = async (req, res, next) => {
    ...
    if (orquery.length > 0) {
        if (gameid && gameid.length > 0) {
            andquery = [{
                "DATE": { $gte: start, $lte: end }, "gameid": mongoose.Types.ObjectId(gameid), USERID: {
                    $in: orquery
                }
            }];
        } else {
...
exports.sattaBymartkethistory = async (req, res, next) => {
    ...
    if (bazaarid && bazaarid.length > 0) {
        ...
        if (gameitem) {
            andquery = [{
                "DATE": { $gte: start, $lte: end }, "bazaarid": mongoose.Types.ObjectId(bazaarid), userid: {
                    $in: orquery
                }
            }];
        } else {
...
exports.sattaBymartketTotal = async (req, res, next) => {
    ...
    andquery = [{ "DATE": { $gte: start, $lte: end }, userid: { $in: orquery } }];
    if (orquery.length > 0) {
...
exports.sattaByBazarhistory = async (req, res, next) => {
    ...
    if (bazaarid && bazaarid.length > 0) {
        andquery = [{ "DATE": { $gte: start, $lte: end }, "type": { $regex: bazaarid }, userid: { $in: orquery } }];
    } else {
        andquery = [{ "DATE": { $gte: start, $lte: end }, userid: { $in: orquery } }];
    }
