...
const { betfairSportslist } = require("../models/betfairmodel")
...
exports.firstpage_load = async (req, res, next) => {
    var totalData = {};
    let list = ["trackcode", "title", "signupbuttons", "logoimg", "footertext", "favicon", "appurl", "TimerButton", "forgotpassword", "DownloadButton", "sportslimit"];
    let query = [];
    ...
    var sidebar = []
    for (let i in row) {
        if (row[i].navLink === "/betexchmain?eventname=Inplay") {
            let array = [
                {
                    pid: row[i]._id,
                    type: "item",
            ...
            for (let j in slist) {
                array.push(Object.assign({}, slist[j]._doc, { pid: row[i]._id },
                    { type: "item" },
                    { navLink: `/betexchmain?eventname=${slist[j].name}` },
                    { id: slist[j]._id },
                    { image: slist[j].icon },
                    { title: slist[j].name }))
            }
...
exports.LivecasinoproviderLoad = async (req, res, next) => {
    ...
    if (!pdata) {
        ...
        if (pdata.length > 0) {
            ...
            var gamelist = [];
            let andquery = { isdelete: false, PROVIDERID: pro[0].provider, status: true }
            if (params) {
            ...
            if (rows.length < 24 && pro.length > 1) {
                for (var i = 1; i < pro.length; i++) {
                    index = i;
                    let andquery1 = { isdelete: false, PROVIDERID: pro[i].provider, status: true }
                    if (params) {
...
exports.LivecasinoproviderChange = async (req, res, next) => {
    ...
    for (var i = 0; i < pro.length; i++) {
        ...
        let andquery = { isdelete: false, PROVIDERID: pro[i].value, status: true }
        if (params) {
            andquery = { ...andquery, ...params }
        }
        var rows = await BASECONTROLL.BSortfind(GAMELISTMODEL, andquery, { order: 1 });
...
exports.scroll_load = async (req, res, next) => {
    ...
    let params = req.body.params
    let andquery = { isdelete: false, PROVIDERID: provider, status: true }
    if (params) {
        andquery = { ...andquery, ...params }
    }
