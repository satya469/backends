...
const { sportsTypeList, sportsBet, sportsTemp, sportsSetting, sportOdds } = require("../models/sports_model")
...
exports.getMinMax = async (req, res) => { // get Min and Max amount
...
exports.setMinMax = async (req, res) => { // set Min and Max amount
    let data = req.body
    let flag = await baseController.BfindOneAndUpdate(sportsSetting, { type: "minmax" },
        {
            type: "minmax",
            data
        }
...
exports.getAdminSportsList = async (req, res) => { // sports list by eventstatus
    let data = req.body
    let query = await sportOdds.find({ EventStatus: data.EventStatus }).distinct("sport_id")
    let list_data = [];
    if (query.length) {
...
exports.getAdminSportData = async (req, res) => { //get matchs for admin website
    let { params, condition } = req.body;
    let newCond = {
        EventStatus: condition.EventStatus
    }
    if (condition.sport_id) {
        newCond.sport_id = condition.sport_id
    }
    if (condition.status == true || condition.status == false) {
        newCond.status = condition.status
    }
    if (condition.event_id) {
        newCond.event_id = condition.event_id
    }
    if (condition.event_name) {
        newCond.event_name = condition.event_name
    }

    let eventData = await baseController.Bfind(sportOdds, newCond)
    let returnData = [];
    for (let i = 0; i < eventData.length; i++) {
        let tempdata = {
            event_id: eventData[i].event_id,
            event_name: eventData[i].event_name,
            ScheduledTime: eventData[i].ScheduledTime,
            permission: eventData[i].permission,
            proData: eventData[i].proData,
            market_len: eventData[i].market.length
        }
...
exports.setAdminMatchStatus = async (req, res) => {// set match's status
    let data = req.body;
    let me = this
    let flag = await baseController.BfindOneAndUpdate(sportOdds, { event_id: data.row.event_id }, { permission: data.key })
    if (flag) {
        return this.getAdminSportData(req, res)
    } else {
        return res.json({ status: false });
    }
}

exports.setAdminFeatureData = async (req, res) => { //add featured events for match
    let row = req.body.row;
    let flag = await baseController.BfindOneAndUpdate(sportOdds, { event_id: row.event_id }, { isfeatured: true })
    if (flag) {
        return res.json({ status: true });
    } else {
        return res.json({ status: false, data: "Failure" });
...
exports.getFeaturedDataAdmin = async (req, res) => { // get featured sports for admin
    return getFeaturedEvents(res)
}

exports.removeFeaturedDataAdmin = async (req, res) => {
    let row = req.body.row;
    let flag = await baseController.BfindOneAndUpdate(sportOdds, { event_id: row.event_id }, { isfeatured: false })
    if (flag) {
...
const getFeaturedEvents = async (res) => { // get featured sports data
    let data = await baseController.Bfind(sportOdds, {
        $and: [
            {
                isfeatured: true,
            },
...
exports.updateProDataOfMatch = async (req, res) => { // update prodata of match
    let data = req.body.data;
    if (data.proData.status) {
        await baseController.BfindOneAndUpdate(sportOdds, { event_id: data.event_id }, { proData: data.proData, permission: false })
        if (data.proData.status == "Suspend" || data.proData.status == "Cancel") {
            setRollbackBets(data.event_id, data.proData.status + " BET")
        }
    } else {
        await baseController.BfindOneAndUpdate(sportOdds, { event_id: data.event_id }, { proData: data.proData, permission: true })
    }
    return this.getAdminSportData(req, res)
}

const setRollbackBets = async (event_id, status) => { // rollback bet by admin
    let bets = await baseController.Bfind(sportsBet, { GAMEID: event_id, "betting.handleState": false })
    for (let k = 0; k < bets.length; k++) {
        let bets_item = bets[k];
        let userData = await baseController.BfindOne(GamePlay, { id: bets_item.USERID });
        if (bets_item.betting.betType.toLowerCase() === "single") {
            ...
            await baseController.player_balanceupdatein_Id(bets_item.AMOUNT, bets_item.USERID, sportsWallet)
            await sportsBet.updateOne({ _id: bets_item._id }, {
                "betting.handleState": true,
                TYPE: status,
                "betting.resultMoney": bets_item.AMOUNT,
                "betting.lastbalance": userData.balance,
                "betting.afterbalance": Number(userData.balance) + Number(bets_item.AMOUNT)
            }).then(async (err) => { });
        } else {
            let sportsWallet = {
                commission: 0,
            ...
            await baseController.player_balanceupdatein_Id(parseFloat(bets_item.AMOUNT), bets_item.USERID, sportsWallet)
            await sportsBet.updateMany({ "betting.transactionId": bets_item.betting.transactionId }, {
                "betting.handleState": true,
                TYPE: status,
                "betting.resultMoney": bets_item.AMOUNT,
                "betting.lastbalance": userData.balance,
...
exports.getAllSportsType = async (req, res) => { // admin sports list cms
...
exports.uploadsportsImage = async (req, res, next) => {// admin sportsbook image upload
...
exports.sportsTypeUpdate = async (req, res) => { // update of sports type
    let indata = req.body.data;
    for (let i = 0; i < indata.length; i++) {
        delete indata[i]._id
        let updatehandle = await baseController.BfindOneAndUpdate(sportsTypeList, { sport_id: indata[i].sport_id }, indata[i]);
        if (!updatehandle) {
            return res.json({ status: false, data: "fail" });
        }
    }
    let findhandle = await baseController.BSortfind(sportsTypeList, {}, { order: 1 });
    if (!findhandle) {
        return res.json({ status: false, data: "fail" })
    } else {
...
exports.selectedMatchAdmin = async (req, res) => {
    let data = req.body
    let rdata = await baseController.BfindOne(sportOdds, { event_id: data.id })
    if (rdata) {
...
exports.getDashboardMarkets = async (req, res, next) => {
    ...
    let betsacount = await sportsBet.countDocuments({ "DATE": { $gte: start, $lte: end } })
...
exports.getBetshistoryload = async (req, res, next) => {
    ...
    let array = await sportsBet.find({
        GAMEID: datap.id,
        DATE: {
            $gte: start,
            $lte: end
        }
    }).populate("gameid").skip(pages.skip).limit(pages.params.perPage).populate("USERID")
    let totoalwallet = {
        pnl: 0,
        "amount of bets": 0,
        "number of bets": 0
...
exports.cancelOneBet = async (req, res, next) => {
    ...
    if (betItem) {
        ...
        if (betItem.TYPE == "BET" || betItem.TYPE == "LOST") {
            ...
            await baseController.BfindOneAndUpdate(sportsBet, { _id: item._id }, {
                TYPE: "CANCEL_BET",
                "betting.handleState": true,
            ...
            await baseController.email_balanceupdate(user.email, betItem.AMOUNT, sportsWallet);
        } else {
            ...
            await baseController.email_balanceupdate(user.email, diffMoney, sportsWallet);
        }
...
exports.updateMtsStatus = async (req, res) => {
    let data = req.body;
    let flag = await baseController.BfindOneAndUpdate(sportsTemp, { key: "systemMts" }, { key: "systemMts", produceStatus: data.systemMts })
    if (flag) {
...
exports.placeBetPlayer = async (req, res) => { // player place bet
    ...
    if (data.betType == "MULTI") {
        ...
        await baseController.player_balanceupdatein_Id(parseFloat(saveData.AMOUNT) * -1, req.user._id, sportsWallet)
        ...
        if (
                ...
                mtsData.bets.push(
                    {
                        ...
                        product_id: data.bet[i].betting.sportid == 21 ? 5 : (
                            data.bet[i].betting.EventStatus == "Live" ? 1 : 3
                        ),
            ...
            instance.get(`http://127.0.0.1:8500/newticket?data=${JSON.stringify(mtsData)}&&&key=1`).then(() => { }).catch(err => { })
        } else {
...
exports.mtsStatusChild = async (sData, io, res) => {
    if (sData) {
        let betData = await baseController.Bfind(sportsBet, { "betting.mtsId": Number(sData.ticketId) });
        if (betData && betData.length) {
            if (sData.status == "true") {
                await sportsBet.updateMany({ "betting.mtsId": Number(sData.ticketId) }, { "betting.mtsState": "accept" })
            } else {
                let amount = 0;
                if (betData[0].betting.betType == "MULTI") {
                ...
                let playerData = await baseController.BfindOne(GamePlay, { id: betData[0].USERID });
                ...
                let userData = await baseController.BfindOne(GamePlay, { id: betData[0].USERID });
                await baseController.player_balanceupdatein_Id(amount, betData[0].USERID, sportsWallet)
                if (sData.iscancel) {
                    await sportsBet.updateMany({ "betting.mtsId": Number(sData.ticketId) }, {
            ...
            let bethandleDatas = await baseController.Bfind(sportsBet, { "betting.transactionId": Number(betData[0].betting.transactionId) });
            let betAcceptStatus = true, accept = 0, reject = 0;
            ...
            let msg = "Success";
            if (betAcceptStatus) {
                let sessionUser = await baseController.BfindOne(sessionmodel, { id: mongoose.Types.ObjectId(betData[0].USERID) });
                if (bethandleDatas.length > 1) {
                    if (reject) {
                        msg = "This bet is rejected"
                        io.sockets.in(sessionUser.socketid).emit("mtsstatus", { status: false, betData, msg });
                    } else {
                        io.sockets.in(sessionUser.socketid).emit("mtsstatus", { status: true, betData, msg });
                    }
                } else {
                    if (!accept) {
                        msg = "This bet is rejected"
...
const checkBetExposureAmount = async (userData, data, userid) => { //exposure compare
    let exposure = 0;
    let d = await sportsBet.aggregate([
        {
            $match: {
                $and: [
                    { USERID: userid, "betting.handleState": false }
                ]
            }
        },
        {
...
const checkBalanceOfUser = async (userData, amount) => { // balance compare
...
exports.getFirstpageData = async (req, res) => {
    let rdata = []
    let query = await sportOdds.find({}).distinct("sport_id")
    let list_data = await sportsTypeList.aggregate([
        {
            $match: {
                $and: [{
                    status: true,
    ...
    async function navCalcData(match) {
        let market = [], marketLen = 0, T1X2;
        if (marketConfig[match.sportid]) {
            T1X2 = match.market.findIndex(item => item.MarketName.toLowerCase().indexOf(marketConfig[match.sportid]) > -1);
        } else {
            T1X2 = match.market.findIndex(item => item.MarketName.toLowerCase() == "1x2");
        }
        let THANDICAP = match.market.findIndex(item => item.MarketName.toLowerCase() == "handicap");
        let TTOTAL = match.market.findIndex(item => item.MarketName.toLowerCase() == "total");
        if (T1X2 > -1) {
    ...
    for (let i = 0; i < list_data.length; i++) {
        let oneSport = list_data[i];
        let sportItems = await baseController.Bfind(sportOdds, { permission: true, sportid: oneSport.sportid })
        for (let i = 0; i < sportItems.length; i++) {
            let oneMatch = sportItems[i];
            if (oneMatch.market && oneMatch.market.length) {
...
exports.getRecoveryEvent = async (req, res) => {
    let data = await baseController.BfindOne(sportsTemp, { key: "RecoveryEvent" });
    if (data) {
        return res.json({ status: true, data })
    } else {
        return res.json({ status: false })
...
exports.getFeaturedEvent = async (req, res) => {
    let returnData = [];
    let values = await baseController.Bfind(sportOdds, { permission: true, isfeatured: true })
    for (let i = 0; i < values.length; i++) {
        let oneMatch = values[i];
        let market = [], T1X2, marketLen = 0;
        if (marketConfig[oneMatch.sportid]) {
            T1X2 = oneMatch.market.findIndex(item => item.MarketName.toLowerCase().indexOf(marketConfig[oneMatch.sportid]) > -1);
        } else {
            T1X2 = oneMatch.market.findIndex(item => item.MarketName.toLowerCase() == "1x2");
        }
        ...
        for (let i = 0; i < oneMatch.market.length; i++) {
            if (oneMatch.market[i].MarketStatus == "Active") {
                for (let j = 0; j < oneMatch.market[i].Outcomes.length; j++) {
        ...
        oneMatch.ScheduledTime = await changeTime(oneMatch.ScheduledTime);
...
exports.getAllSportsEventsByEventStatus = async (req, res) => {
    ...
    let values = await baseController.Bfind(sportOdds, { EventStatus, permission: true })
    let items = [];
    for (let i in values) {
        let item = JSON.parse(values[i]);
        let market = [], T1X2, marketLen = 0;
        if (marketConfig[item.sportid]) {
            T1X2 = item.market.findIndex(a => a.MarketName.toLowerCase().indexOf(marketConfig[item.sportid]) > -1);
        } else {
            T1X2 = item.market.findIndex(a => a.MarketName.toLowerCase() == "1x2");
        }
        let THANDICAP = item.market.findIndex(item => item.MarketName.toLowerCase() == "handicap");
        let TTOTAL = item.market.findIndex(item => item.MarketName.toLowerCase() == "total");
        if (T1X2 > -1) {
...
exports.getSportsListPlayerChild = async (EventStatus, cb) => {
    let values = await baseController.Bfind(sportOdds, {})
    let list_data = [];
    for (let i = 0; i < values.length; i++) {
        let oneMatch = values[i];
        let ScheduledTime
        if (oneMatch.ScheduledTime && oneMatch.ScheduledTime.indexOf("IST") > -1) {
            ScheduledTime = new Date(oneMatch.ScheduledTime.replace("IST", "GMT +05:30")).valueOf();
        } else {
            ScheduledTime = new Date(oneMatch.ScheduledTime).valueOf();
        }

        if (
            (oneMatch.EventStatus != "Live" && oneMatch.EventStatus != "NotStarted" && oneMatch.EventStatus != "Suspended")
            || (oneMatch.Status && oneMatch.Status.MatchStatus == "Ended")
            || (oneMatch.EventStatus == "NotStarted" && ScheduledTime < (new Date().valueOf() - (24 * 60 * 60 * 1000)))
            || (oneMatch.EventStatus == "Suspended" && ScheduledTime < (new Date().valueOf() - (24 * 60 * 60 * 1000)))
            || (oneMatch.EventStatus == "Live" && ScheduledTime < (new Date().valueOf() - (24 * 60 * 60 * 1000)))
        ) {
            baseController.BfindOneAndDelete(sportOdds, { event_id: oneMatch.event_id, sportid: oneMatch.sportid })
        }

        if ((EventStatus == "sports" || oneMatch.EventStatus == EventStatus) && oneMatch.permission) {
            let index = list_data.findIndex(item => item.sport_id == oneMatch.sportid);
            if (index < 0) {
                let tempList = await baseController.BfindOne(sportsTypeList, { status: true, sport_id: oneMatch.sportid });
                if (tempList) {
...
exports.getSportsMatchPlayer = async (req, res) => {
    let data = req.body;
    let returnData = [];
    let oddsData = await baseController.Bfind(sportOdds, { sportid: data.sportid, permission: true })

    async function navCalcData(match) {
        let market = [], marketLen = 0;
        let T1X2;
        if (marketConfig[data.sportid]) {
            T1X2 = match.market.findIndex(item => item.MarketName.toLowerCase().indexOf(marketConfig[data.sportid]) > -1);
        } else {
            T1X2 = match.market.findIndex(item => item.MarketName.toLowerCase() == "1x2");
        }
        ...
        match.market = market;
        match.ScheduledTime = await changeTime(match.ScheduledTime);
        let index = returnData.findIndex(item => item.event_id === match.event_id);
        if (index > -1) {
            returnData[index] = match;
            baseController.BfindOneAndDelete(sportOdds, { event_id: returnData[index].event_id, sportid: returnData[index].sportid })
        } else {
    ...
    for (let i = 0; i < oddsData.length; i++) {
        let oneMatch = oddsData[i];
        if (oneMatch.market.length) {
            if (data.EventStatus == "Live" && oneMatch.EventStatus == "Live") {
                await navCalcData(oneMatch);
            } else if (data.EventStatus == "NotStarted" && oneMatch.EventStatus == "NotStarted") {
                await navCalcData(oneMatch);
            } else if (data.EventStatus == "Next24") {
                if (oneMatch.ScheduledTime && oneMatch.ScheduledTime.indexOf("IST") > -1) {
                    let ScheduledTime = new Date(oneMatch.ScheduledTime.replace("IST", "GMT +05:30"));
...
exports.getOneMatchPlayer = async (req, res) => {
    let rdata = await baseController.BfindOne(sportOdds, { event_id: req.body.event_id })
    if (rdata) {
        rdata.ScheduledTime = await changeTime(rdata.ScheduledTime);
        return res.json({ status: true, data: rdata });
    } else {
...
exports.cashOut = async (req, res) => {
    let data = req.body;
    await baseController.Bfind(sportsBet, { "betting.transactionId": Number(data.betting.transactionId) });
    ...
    for (let i = 0; i < data.bet.length; i++) {
        ...
        instance.get(`http://127.0.0.1:8500/newticket?data=${JSON.stringify(mtsData)}`).then(() => { }).catch(err => { })
    }
}
...
exports.getSportsBetHistory = async (req, res) => {
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if (result[i].betting.matchTime) {
            result[i].betting.matchTime = new Date(result[i].betting.matchTime).toLocaleString();
        }
    }
    if (result) {
...
exports.cMtsStatus = async (req, res) => {
    ...
    await baseController.BfindOneAndUpdate(sportsBet, { "betting.mtsId": Number(ticketId) }, { "betting.iscancel": iscancel })
    return res.send(true)
}
...
exports.mtsConnectionStatus = async (req, res) => {
    ...
    if (fdata.isDisconnected == "true") {
        ...
        let bets = await baseController.BfindOne(sportsBet, { "betting.mtsId": Number(ticketid) })
        await sportsBet.updateMany({ "betting.mtsId": Number(ticketid) }, { "betting.mtsSent": false })
        let sessionUser = await baseController.BfindOne(sessionmodel, { id: mongoose.Types.ObjectId(bets.USERID) });

        baseController.BfindOneAndUpdate(sportsTemp, { key: "mts" }, { produceStatus: false, key: "mts" })

        io.sockets.in(sessionUser.socketid).emit("mtsstatus", {
            status: false,
            mts: true,
            msg: "Failure (mts)",
        ...
        baseController.BfindOneAndUpdate(sportsTemp, { key: "mts" }, { produceStatus: true, key: "mts" })
        ...
        await sportsBet.updateMany({ "betting.mtsState": "pending" }, { "betting.mtsSent": true })

        for (let key = 0; key < bets.length; key++) {
            let data = bets[key];
            let flag = temp.findIndex(item => item._id == data._id)
            if (flag < 0) {
                if (data.betting.betType == "MULTI") {
                    temp.push(data)
                    let mtsData = {
                        ip: data.betting.ip,
                        userid: data.USERID,
                            ...
                            mtsData.bets.push(
                                {
                                    ...
                                    product_id: bets[i].betting.sportid == 21 ? 5 : (
                                        bets[i].betting.EventStatus == "Live" ? 1 : 3
                                    ),
                    ...
                    mtsData.bets.push(
                        {
                            ...
                            eventid: data.GAMEID,
                            product_id: data.betting.sportid == 21 ? 5 : (
                                data.betting.EventStatus == "Live" ? 1 : 3
                            ),
...
exports.ticketCancel = async (req, res) => {
    ...
    instance.get(`http://127.0.0.1:8500/newticket?data=${JSON.stringify(sendData)}&&&key=2`).then(() => { })
    return res.send(true);
}
