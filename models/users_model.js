...
const users = () =>{
    ...
    UserSchema.pre('find', function () {
        ...
        this.populate('playerid').populate('permissionid',["title","id","pid"]);
    ...
    UserSchema.pre('findOne', function () {
        // `this` is an instance of mongoose.Query
        this.populate('playerid',["balance","bonusbalance","sattalimit","exchangelimit","sportsbooklimit","betdelaytime","livecasino","casino","skill"]).populate('permissionid',["title","id","pid"]);
        this.select(["username","email","password",
            "lastname",
            "firstname",
            "status",
...
const operators = () =>{
    ...
    UserSchema.pre('save', function() {
        this.set({ operatorid: get_max_id() });
        this.set({ date: basecontroller.Indiatime() });
    });

    
    return mongoose.model("user_operators", UserSchema);
}
...
const changepasswordhistory = () =>{
    ...
    return mongoose.model("changepasswordhistory", UserSchema)
}
