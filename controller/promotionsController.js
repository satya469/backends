...
exports.getBonusTotal = async (req, res, next) => {
    ...
    if (orquery.length > 0) {
        ...
        andquery = [{ createdAt: { $gte: start, $lte: end }, userid: { $in: orquery } }];
...
exports.getBonusHistory = async (req, res, next) => {
    ...
    if (params && filters) {
        ...
        var andquery = {};
        if (bonusid && bonusid.length) {
            andquery = { userid: { $in: orquery }, createdAt: { $gte: start, $lte: end }, bonusid: mongoose.Types.ObjectId(bonusid) };
        } else {
            andquery = { userid: { $in: orquery }, createdAt: { $gte: start, $lte: end } };
        }

        let totalcount = await BonusHistory.countDocuments(andquery);
        var pages = reportcontrol.setpage(params, totalcount);
        let array = await BonusHistory.find(andquery).sort({ createdAt: -1 }).skip(pages.skip).limit(pages.params.perPage).populate("userid").populate("bonusid");
        ...
        for (var i in array) {
            ...
            if (end.getTime() > new Date().getTime()) {
                ...
                let wager = item.amount * bitem.wager;
                let remaind = wager - Achivedamount;
                let playeritem = BASECONTROLL.getPlayerBalanceCal(item.userid.playerid)
                
                if (remaind <= 0 || (Number(item.amount) > Number(playeritem.bonusbalance) )) {
                // if (remaind > 0) {
                    row['accept'] = false;
                } else {
                    if (item.status != "1") {
...
exports.bonus_menuload = async (req, res, next) => {
    ...
    if (params) {
        ...
        var pages = reportcontrol.setpage(params, totalcount);
        let array = await BonusMenuModel.find({ isdelete: false }).sort().skip(pages.skip).limit(pages.params.perPage);
        pages["skip2"] = (pages.skip) + array.length;
...
exports.Claim_request = async (req, res, next) => {
    ...
    let playeritem = BASECONTROLL.getPlayerBalanceCal(req.user.playerid)

    let update = await this.AcceptBonus(bonusitem, playeritem);
    if (update) {
...
exports.AcceptBonus = async (bonusitem, playeritem) => {

    // console.log("processss ---------1")
    ...
    // console.log("processss ---------2")
        ...
        if (playeritem.bonusbalance >= amount) {
            // console.log("processss ---------3")
        ...
        // console.log("processss ---------6")
                return true
            } else {
        // console.log("processss ---------7")
                return false;
            }
        } else {
            // console.log("processss ---------4")
            return false;
        }

    } else {
        // console.log("processss ---------5")
        return false;
    }
}
...
exports.CreditBonus = async (req, res, next) => {
    ...
    if (bonusitem) {
        let playeritem = BASECONTROLL.getPlayerBalanceCal(bonusitem.userid.playerid)
...
exports.bonus_menuloads = async (req, res, next) => {
    ...
    let array = [];
    // console.log("----process 1", his.length)
    for (var i in his) {
        let item = his[i]._doc;
        // console.log(item)
        let start = new Date(item['createdAt']);
        ...
        if (end.getTime() > new Date().getTime()) {
            ...
            if (remaind <= 0 ) {
                ...
                // console.log(playeritem.bonusbalance,"playeritem.bonusbalance")

                if (Number(item.amount) <= Number(playeritem.bonusbalance) ) {
                    // console.log(true,"------------------")
                    row['accept'] = true
                    array.push(row)
                } else {
