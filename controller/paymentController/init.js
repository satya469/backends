...
exports.Loadresstrictiondays = async (req, res, next) => {
    ...
    if (params && filters) {
        ...
        if (totalcount > 0) {
            array = await resstrictionDays.find({ type: filters }).skip(pages.skip).limit(pages.params.perPage);
        }
...
exports.getbankdetail = async (req, res, next) => {
    ...
    if (params && filters) {
        ...
        if (orquery.length > 0) {
            let totalcount = await paymentuserdetail.countDocuments({ paymentconfigid: mongoose.Types.ObjectId(cashconfigid), userid: { $in: orquery} });
            pages = reportsControl.setpage(params, totalcount);
            if (totalcount > 0) {
                array = await paymentuserdetail.find({ paymentconfigid: mongoose.Types.ObjectId(cashconfigid), userid: {$in: orquery} }).skip(pages.skip).limit(pages.params.perPage).populate("paymentconfigurationid").populate("userid");
            }
        } else {
            pages = reportsControl.setpage(params, 0);
        }
...
exports.getPayoutchannel = async (req, res, next) => {
    ...
    if (params) {
        ...
        if (totalcount > 0) {
            array = await payoutchannel.find().skip(pages.skip).limit(pages.params.perPage).populate("paymentconfigurationid");
        }
...
exports.playermenuloads = async (req, res, next) => {
    ...
    if (!findhandle) {
        ...
        for (let i in findhandle) {
            ...
            delete row.paymentconfigurationid
            rows.push(row);
        }
