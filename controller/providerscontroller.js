...
const FIRSTPAGE_GEMLIST_Model =
  require("../models/games_model").FIRSTPAGE_GAMELIST_MODEL;
...
exports.LivecasinoproviderLoad = async (req, res, next) => {
  ...
  if (bool && params) {
    ...
    if (providerid && providerid.length > 0) {
      ...
      let orquery = [];
      for (var i in pitems) {
        orquery.push(mongoose.Types.ObjectId(pitems[i]._id));
      }
...
exports.LivecasinoproviderChange = async (req, res, next) => {
  ...
  var gamelist = await BASECONTROL.BSortfind(
    GAMELISTMODEL,
    { isdelete: false, PROVIDERID: pro },
    { order: 1 }
  );
  if (gamelist) {
...
exports.LivecasinoProviderCheck = async (req, res, next) => {
  ...
  if (filters) {
    if (filters.providerid && filters.providerid.length > 0) {
      var updatehandle = await GAMELISTMODEL.updateMany(
        { providerid: mongoose.Types.ObjectId(filters.providerid) },
        { status: handle }
      );
      if (updatehandle) {
...
exports.Livecasinostatuspagecheck = async (req, res, next) => {
  ...
  var handle = req.body.handle;
  var updatehandle = await GAMELISTMODEL.findOneAndUpdate(
    { _id: row._id },
    { status: handle }
  );
  if (updatehandle) {
...
exports.topgamescheck = async (req, res, next) => {
  ...
  if (row && bool) {
    var data = await BASECONTROL.BfindOne(TopGamelistmodel, {
      gameid: mongoose.Types.ObjectId(row._id),
    });
    if (!data) {
      if (handle) {
        var item = {};
        var alllist = await BASECONTROL.BSortfind(
          TopGamelistmodel,
          {},
          { order: -1 }
        );
...
exports.LivecasinoFirstPageCheck = async (req, res, next) => {
  ...
  if (row && bool) {
    var data = await BASECONTROL.BfindOne(FIRSTPAGE_GEMLIST_Model, {
      gameid: mongoose.Types.ObjectId(row._id),
    });
    if (!data) {
      if (handle) {
        ...
        item["type"] = bool;
        item["gameid"] = mongoose.Types.ObjectId(row._id);
        var shandle = await BASECONTROL.data_save(
          item,
          FIRSTPAGE_GEMLIST_Model
        );
        if (shandle) {
...
exports.get_firstpage_gamelist = async (req, res, next) => {
  ...
  var rows = await BASECONTROL.BSortfindPopulate(
    FIRSTPAGE_GEMLIST_Model,
    { type: type },
    { order: 1 },
    "gameid"
...
exports.delete_firstpage_gamelist = async (req, res, next) => {
  var data = req.body.data;
  if (data) {
    var udata = await BASECONTROL.BfindOneAndDelete(FIRSTPAGE_GEMLIST_Model, {
      _id: data._id,
    });
    if (udata) {
      this.get_firstpage_gamelist(req, res, next);
...
exports.Livecasinoitemsadd = async (req, res, next) => {
  ...
  var fhandle = await BASECONTROL.BfindOne(GAMELISTMODEL, {
    providerid: mongoose.Types.ObjectId(providerid),
    ID: ID,
  });
  if (fhandle) {
    ...
    if (pdata) {
      let order = 0;
      let topitem = await GAMELISTMODEL.find({
        isdelete: false,
        providerid: mongoose.Types.ObjectId(providerid),
      })
        .sort({ order: -1 })
...
exports.Livecasinoitemsdelete = async (req, res, next) => {
  ...
  var fhandle = await BASECONTROL.BfindOneAndUpdate(
    GAMELISTMODEL,
    { providerid: mongoose.Types.ObjectId(providerid), ID: ID },
    { isdelete: true }
  );
  if (fhandle) {
    this.LivecasinoproviderLoad(req, res, next);
...
exports.Livecasinoitemsimg_upload = async (req, res, next) => {
  ...
  if (imagesrc) {
    ...
    if (gameitem) {
      ...
      let update = await BASECONTROL.BfindOneAndUpdate(
        GAMELISTMODEL,
        { _id: _id },
        { image: imagesrc }
      );
...
async function onMessage(evt) {
  ...
  switch (MessageType) {
    ...
    case "DealerChanged":
      ...
      var fdata = await BASECONTROL.BfindOne(GAMELISTMODEL, {
        LAUNCHURL: "3",
        ID: TableId,
      });
      var uhandle = await BASECONTROL.BfindOneAndUpdate(
        GAMELISTMODEL,
        { LAUNCHURL: "3", ID: TableId },
        { NAME: DealerName }
      );
...
async function euzgi_update(TablesList) {
  ...
  for (var i = 0; i < TablesList.length; i++) {
    ...
    row["LAUNCHURL"] = "3";
    row["image"] =
      "https://recording.ezugicdn.com/Dealer-Images/Single/" +
      TablesList[i].TableId +
      ".jpg";
...
exports.allrefreshGames = (req, res, next) => {
  ...
  betsoftrefreshgames("BETSOFT", async (rdata) => {
    for (var i = 0; i < rdata.length; i++) {
      var findhandle = await BASECONTROL.BfindOne(GAMELISTMODEL, {
        ID: rdata[i].ID,
        PROVIDERID: rdata[i].PROVIDERID,
      });
      if (!findhandle) {
        var savehandle = await BASECONTROL.data_save(rdata[i], GAMELISTMODEL);
      } else {
        var updatehandle = await BASECONTROL.BfindOneAndUpdate(
          GAMELISTMODEL,
          { ID: rdata[i].ID, PROVIDERID: rdata[i].PROVIDERID },
          rdata[i]
        );
      }
    }
...
async function vivo_refreshgames(provider, callback) {
  ...
  async function get_axois(gameName, PROVIDERID) {
    ...
    await axios
      .get(
        BASECONFIG.vivo.gamelist +
          "mobile=true&operatorId=" +
      ...
      .then((rdata) => {
        ...
        for (var i in tbldata) {
          ...
          for (let j in tbldata[i].limits) {
            ...
            limits.push({
              limitId: jj.limitid,
              limitMin: jj.limitMin,
              limitMax: jj.limitMax,
            });
          }
          var newrow = {};
  ...
  axios(config)
    .then(async function (response) {
      ...
      for (var i = 0; i < providers.length; i++) {
        ...
        var type = {};
        let providerid = "";
        var findhandle = await BASECONTROL.BfindOne(PROVIDERMODELS, {
          provider: providername,
        });
        console.log(providername);
        if (findhandle) {
          providerid = findhandle._id;
        } else {
          let p = {
            provider: providername,
            text: providername,
          ...
          var savehandle = await BASECONTROL.data_save(p, PROVIDERMODELS);
          if (savehandle) {
            providerid = savehandle._id;
          }
        }
...
function Goldenrace() {
  var options = {
    method: "POST",
    url: "https://fairbetsco.api-hub.xpressgaming.net/api/v3/get-game-list",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
    form: {
      siteId: "5624",
      publicKey: "KGZ3V6ajBayJipZ",
    },
  };
  request(options, async function (error, response) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].provider == "GOLDENRACE") {
        ...
        row["isdelete"] = false;
        row["status"] = true;
        row["providerid"] = mongoose.Types.ObjectId("5f81e04664fe8c3930d36ea5");
        index++;
        ...
        var dd = await BASECONTROL.BfindOne(
          GAMELISTMODEL,
          { ID: row.ID, PROVIDERID: row.PROVIDERID },
          row
        );
        if (!dd) {
          var allpro = await GAMELISTMODEL.findOne({
            PROVIDERID: row.PROVIDERID,
          }).sort({ order: -1 });
          var order = allpro.order + 1;
          row["order"] = order;
...
function myslotty() {
  ...
  request(options, async function (error, response) {
    ...
    for (var i in providers) {
      var fd = await BASECONTROL.BfindOneAndUpdate(
        PROVIDERMODELS,
        { provider: i, LAUNCHURL: "8" },
        { Agregator: "MySlotty" }
      );
      if (!fd) {
...
exports.evoplay = async (req, res, next) => {
  ...
  request(options, async function (error, response) {
    ...
    for (var i in indata) {
      ...
      row["isdelete"] = false;
      row["status"] = true;
      row["providerid"] = mongoose.Types.ObjectId("5fb4a0e751a6614de728ad4b");
      index++;
